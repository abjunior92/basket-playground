...
export const loader = async ({ params }: LoaderFunctionArgs) => {
	...
	return json({ format: resolveTournamentFormat(playground.format) })
}

export default function Rules() {
	const params = useParams()
	const { format } = useLoaderData<typeof loader>()
	const formatRules = getFormatRulesSections(format)
	...
	const faqSections = [
		...
		},
		{
			...
			items: [
				...
				'Nei Play-in e Playoff, gli arbitri gestiscono integralmente la gara: la decisione arbitrale è sovrana e non contestabile.',
				'Nelle partite della fase a gironi c’è l’auto-arbitraggio tra le squadre. In caso di problemi prenderà le decisioni uno dello staff presente nel campo. Si riservano valutazioni in caso di comportamenti scorretti e poco sportivi.',
